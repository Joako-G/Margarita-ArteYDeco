...
export function AdminConsumerWithdrawalsPage() {
  ...
  return (
    <main aria-labelledby="admin-withdrawals-title" className="admin-page admin-withdrawals">
      <AdminPageHeader actions={<Button onClick={openContingency}>Cargar desde WhatsApp</Button>} currentLabel="Arrepentimientos" description="Gestioná las solicitudes de devolución y reintegro." sectionLabel="Gestión" title="Arrepentimientos" titleId="admin-withdrawals-title" />
      <section aria-labelledby="withdrawal-filters-title" className="admin-withdrawals__panel">
        <div className="admin-withdrawals__heading"><h2 id="withdrawal-filters-title">Filtros</h2></div>
      ...
      <section aria-labelledby="withdrawal-list-title" className="admin-withdrawals__panel admin-withdrawals__list-panel">
        <div className="admin-withdrawals__heading"><h2 id="withdrawal-list-title">Solicitudes</h2>{pagination ? <span aria-live="polite" className="admin-withdrawals__result-count">{pagination.totalItems} {pagination.totalItems === 1 ? 'solicitud' : 'solicitudes'}</span> : null}</div>
        ...
        {requests.data?.items.length ? <div className="admin-withdrawals__table" role="region" aria-label="Listado de solicitudes"><table><thead><tr><th>Código</th><th>Fecha</th><th>Estado</th><th>Pedido</th><th>Revisar antes de</th><th>Acciones</th></tr></thead><tbody>{requests.data.items.map((item) => <tr key={item.id}><td data-label="Código"><strong>{item.administrativeCode}</strong></td><td data-label="Fecha">{formatConsumerWithdrawalDate(item.submittedAt)}</td><td data-label="Estado"><Badge variant={statusVariant(item.requestStatus)}>{REQUEST_STATUS_LABELS[item.requestStatus]}</Badge>{item.isDeadlineReviewRequired ? <span className="admin-withdrawals__attention">Revisar fechas</span> : null}</td><td data-label="Pedido">{item.order?.orderNumber ?? 'Sin identificar'}</td><td data-label="Revisar antes de">{item.firstReviewDueAt ? formatConsumerWithdrawalDate(item.firstReviewDueAt) : 'Sin fecha límite'}</td><td data-label="Acciones"><Link className="admin-withdrawals__detail-link" to={routes.adminConsumerWithdrawalDetail(item.id)}><Eye aria-hidden="true" size={17} />Ver detalle</Link></td></tr>)}</tbody></table></div> : null}
        {requests.data?.items.length === 0 ? <EmptyState description="Probá con otros filtros." icon={<RotateCcw />} title="No hay solicitudes" /> : null}
