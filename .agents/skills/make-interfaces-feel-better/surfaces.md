...
## Documented Shadows and Borders

For Margarita Arte & Deco, preserve the single soft elevation defined in `docs/DESIGN-SYSTEM.md`. Cards do not use visible borders; inputs and focus indicators keep their documented borders. Do not replace one treatment with another unless the Design System is updated first.
...
Reference the documented elevation through a CSS variable. If the variable is missing from `frontend/src/styles/variables.css`, propose adding it before implementation instead of copying a raw shadow value.
...
- Use an existing Design System token.
- Do not add a hardcoded outline color.
- If no suitable token exists, report the proposed addition before implementing it.
...
Interactive elements must have a minimum hit area of 44×44px. If the visible element is smaller (e.g., a 20×20 checkbox), extend the hit area with a pseudo-element.
...
Implement the expanded hit area through a reusable CSS class; do not add Tailwind utilities.
