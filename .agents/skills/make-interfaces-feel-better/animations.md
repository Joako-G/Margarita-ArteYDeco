...
## Margarita project override

Use the examples in this file as interaction patterns, not as copy-ready values. Implement with Framer Motion or existing CSS classes, use only documented tokens, keep each complete sequence within 200–500 ms, support `prefers-reduced-motion`, and never animate disabled controls. Do not introduce Tailwind or another motion dependency.
...
2. **Stagger** only when the complete sequence remains within 500 ms
...
A subtle press state can provide tactile feedback when the Design System defines it. Do not introduce an undocumented scale value. Use interruptible transitions so the control returns smoothly when the user releases it.
