...
const PRODUCT_NAMES = ['PRODUCT.md', 'Product.md', 'product.md', 'BUSINESS-RULES.md'];
const DESIGN_NAMES = ['DESIGN.md', 'Design.md', 'design.md', 'DESIGN-SYSTEM.md'];
...
async function cli() {
  ...
  if (!ctx.hasProduct) {
    ...
    const parts = [
      'NO_PROJECT_CONTEXT: docs/BUSINESS-RULES.md was not found. ' +
      'Report the missing authoritative project documentation and do not create PRODUCT.md or DESIGN.md.',
    ];
  ...
  const productLabel = ctx.productPath ? path.basename(ctx.productPath) : 'PRODUCT.md';
  const parts = [`# ${productLabel}\n\n${ctx.product.trim()}`];
  if (ctx.hasDesign) {
    const designLabel = ctx.designPath ? path.basename(ctx.designPath) : 'DESIGN.md';
    parts.push(`# ${designLabel}\n\n${ctx.design.trim()}`);
  }
  parts.push(buildResolvedContextDirective(ctx, cliOptions, { targetExists }));
  if (shouldWarnMissingTarget(ctx, targetProvided, targetExists)) {
    parts.push(buildMissingTargetDirective());
  ...
  const next = register
    ...
    : `NEXT STEP: You MUST now read the matching register reference (\`reference/brand.md\` or \`reference/product.md\`) before producing any design output. Pick based on the requested surface and the project context above.`;
  parts.push(next);
  if (updateDirective) parts.push(updateDirective);
  process.stdout.write(parts.join('\n\n---\n\n') + '\n');
}
