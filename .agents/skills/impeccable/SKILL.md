---
name: impeccable
description: Design, critique, audit, polish, adapt, animate, or otherwise improve the Margarita Arte & Deco React interface. Use for the public ecommerce, admin panel, layouts, components, forms, responsive behavior, accessibility, performance, typography, spacing, color, motion, UX copy, states, and design-system consistency. Follow AGENTS.md and docs/ as the source of truth; do not use for backend-only tasks.
---
...
- Read `AGENTS.md` first and follow its documentation priority.
- Treat `docs/BUSINESS-RULES.md`, `docs/DESIGN-SYSTEM.md`, and `docs/CONVENTIONS.md` as mandatory context.
- Read `docs/FRONTEND-SDD.md` for public UI, `docs/ADMIN-SDD.md` for admin UI, and `docs/CONTENT-STRATEGY.md` whenever visible copy is involved.
- Read backend or database specifications only when a UI decision depends on an API contract or persisted field.
- Never create `PRODUCT.md` or `DESIGN.md`; their responsibilities already belong to the documents above.
- Never change the documented stack, architecture, dependencies, business rules, tokens, or visual language. Report an inconsistency or improvement first and wait for authorization before changing documentation.
- Use Spanish for user-facing copy and English for code.

## Setup

You MUST do these steps before proceeding:

1. Run `node .agents/skills/impeccable/scripts/context.mjs` once per session. If `node` is unavailable on `PATH`, use the Node.js runtime exposed by the Codex workspace. The adapted loader resolves `docs/BUSINESS-RULES.md` and `docs/DESIGN-SYSTEM.md`; do not enter the upstream `init` flow.
...
3. Read the task-relevant project documents listed under Project authority and inspect at least one representative component plus `frontend/src/styles/variables.css`.
4. Read `reference/brand.md` for the landing, catalog, public checkout, or marketing content. Read `reference/product.md` for `/admin`, dashboards, forms, tables, and operational flows.
5. Preserve the existing palette, typography, radii, spacing, shadows, breakpoints, icon library, and motion rules. Do not run `palette.mjs` for this project.
...
- Use Framer Motion for advanced motion. Do not add another animation dependency.
- Keep motion within the documented 200–500 ms range and never animate disabled controls.
...
### New projects only (not applicable to Margarita Arte & Deco)

#### Color & Theme

- Do not apply this section to this repository. Preserve the HEX tokens and light-only theme defined in `docs/DESIGN-SYSTEM.md`.
...
- **Undocumented radii.** Use the exact component radii from `docs/DESIGN-SYSTEM.md` (including 20px cards and 28px drawers). Do not invent alternatives; full-pill remains appropriate only where documented.
...
1. **No argument**: the user is asking "what should I do?" Make the menu context-aware instead of static. Run `context-signals.mjs` only if it can operate without creating `PRODUCT.md` or `DESIGN.md`, then lead with the **2-3 highest-value next commands**, each with a one-line reason pulled from the available signals, followed by the full menu. **Never auto-run a command; the recommendation is a suggestion the user confirms.**

   Reason over the signals; there is no score to obey:
   - `setup.hasDesign` false while `setup.hasCode` true → inspect `docs/DESIGN-SYSTEM.md`; do not generate another design document.
...
If the first word is `craft`, setup still runs first, but [reference/craft.md](reference/craft.md) owns the rest of the flow subject to Project authority.

If the user invokes `init`, `document`, or `teach`, explain that this repository already has authoritative product and design documentation. Offer to review the corresponding files in `docs/` instead of creating duplicates.
